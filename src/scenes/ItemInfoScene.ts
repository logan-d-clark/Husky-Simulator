...
interface InfoData { type: ItemType; resume: () => void }
...
export class ItemInfoScene extends Phaser.Scene {
  ...
  constructor() { super('ItemInfo'); }
  init(data: InfoData) { this.info = data; }
  ...
  create() {
    ...
    panel.fillStyle(Phaser.Display.Color.HexStringToColor('#241f1b').color, 0.98)
      .fillRoundedRect(cx - W / 2, cy - H / 2, W, H, 14);
    panel.lineStyle(2, Phaser.Display.Color.HexStringToColor(PALETTE.treat).color, 1)
      .strokeRoundedRect(cx - W / 2, cy - H / 2, W, H, 14);

    this.add.text(cx, cy - H / 2 + 28, 'NEW ITEM', {
      fontSize: '13px', color: '#d8b06a', fontStyle: 'bold',
    }).setOrigin(0.5).setLetterSpacing(2);

    this.add.image(cx, cy - 62, this.info.type).setDisplaySize(46, 51);
    this.add.text(cx, cy - 22, info.name, { fontSize: '26px', color: '#ffd27f', fontStyle: 'bold' }).setOrigin(0.5);
    this.add.text(cx, cy + 32, info.blurb, {
      fontSize: '15px', color: PALETTE.hudText, align: 'center', wordWrap: { width: W - 80 },
    }).setOrigin(0.5);
    this.add.text(cx, cy + H / 2 - 46, `Press  ${info.key}  to use it`, {
      fontSize: '17px', color: '#ffd27f',
    }).setOrigin(0.5);
    this.add.text(cx, cy + H / 2 - 20, 'click or press any key to continue', {
      fontSize: '12px', color: '#b7a482',
    }).setOrigin(0.5);
    ...
    this.time.delayedCall(350, () => {
      ...
      this.input.keyboard?.on('keydown', (e: KeyboardEvent) => { if (!e.repeat) dismiss(); });
    });
  }
}
