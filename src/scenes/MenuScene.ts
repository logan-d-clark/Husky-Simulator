...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      ...
      .setOrigin(0.5)
      .setShadow(0, 2, 'rgba(255,255,255,0.42)', 0, false, true);
    this.add
      .text(cx, 192, 'Escape the yard. Gather treats. Rule the neighborhood.', {
        fontFamily: DISPLAY_FONT,
        fontSize: '20px',
        color: '#38505b',
        fontStyle: 'bold',
    ...
    this.add
      .image(W - 210, 476, 'menu-chi')
      .setScale(0.5)
      .setOrigin(0.5);

    // Difficulty segmented control.
    this.add
      .text(cx, layout.challengeHeadingY, 'CHOOSE YOUR CHALLENGE', {
        fontFamily: DISPLAY_FONT,
        fontSize: '15px',
        color: '#5e492c',
        fontStyle: 'bold',
  ...
  private drawScene(W: number, H: number, horizon: number) {
    ...
    // Sky gradient.
    g.fillGradientStyle(hex('#8fd4ec'), hex('#8fd4ec'), hex('#cdeef7'), hex('#cdeef7'), 1).fillRect(
      0,
      0,
      W,
      horizon,
    ...
    for (const [x, y, s] of [
      ...
      g.fillCircle(x + 26 * s, y + 6, 28 * s);
      g.fillCircle(x + 58 * s, y, 20 * s);
    }
  ...
  private buildDifficulty(cx: number, y: number) {
    ...
    const segW = 150,
      segH = DIFFICULTY_SEGMENT_HEIGHT,
      gap = 10,
      totalW = levels.length * segW + (levels.length - 1) * gap;
    ...
    const refresh = () => {
      for (const s of segs) {
        ...
        s.g.clear();
        s.g
          .fillStyle(active ? hex('#ffd27f') : hex('#2a2622'), active ? 1 : 0.55)
          .fillRoundedRect(-segW / 2, -segH / 2, segW, segH, 10);
        s.g
          .lineStyle(1.5, hex('#ffd27f'), active ? 1 : 0.4)
          .strokeRoundedRect(-segW / 2, -segH / 2, segW, segH, 10);
    ...
    levels.forEach((key, i) => {
      ...
      const name = this.add
        .text(0, -9, label, { fontSize: '18px', color: '#f2ede0', fontStyle: 'bold' })
        .setOrigin(0.5);
      const sub = this.add
        .text(0, 11, DIFFICULTIES[key].subtitle, { fontSize: '11px', color: '#b7a482' })
        .setOrigin(0.5);
      cont.add([g, name, sub]);
  ...
  private buildDevToggle(x: number, y: number) {
    ...
    const draw = () => {
      g.clear();
      g.fillStyle(this.devMode ? hex('#8fd98f') : hex('#2a2622'), 0.9).fillRoundedRect(
        -w / 2,
        -h / 2,
        w,
        h,
