...
- **Rates are shared with Blizzard** — no Bandit-specific water/poop/pee rate.
  ...
  (POOP_RATE/PEE_RATE per tick) are reused as-is; only *when/where/how long* he
  does them changes.
...
| "Take as long as Blizzard to replenish" | `inferable` | inference | Interpreted as: use Blizzard's shared rates (no special-casing) and commit to a *full* refill / real relieving over multiple ticks — the perceived "one-tick refill" is really leaving early, not a faster rate. |
