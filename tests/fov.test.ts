...
function mapFrom(rows: string[], fences: [number, number, 'top' | 'bottom' | 'left' | 'right'][] = []): GameMap {
  const T: Record<string, TileType> = { G: 'grass', H: 'house', P: 'pavement', W: 'water' };
  const tiles: Tile[][] = rows.map((line, r) =>
    line.split('').map((ch, c) => ({
      col: c, row: r, type: T[ch], ownerId: 0, fences: emptyFences(),
      heat: 0, dirt: 0, destruction: 0, foodPresent: false,
    })),
  );
